...
type ExportableIssue = Omit<Issue, "startDate"> & { startDate?: Dayjs, endDate?: Dayjs, workingDays: number }

const addExportedTimeProperties = (issue: Issue, inProgressStatusNames: string[]): ExportableIssue => {
  const statusItems = issue.changelog.histories.reverse()
    .map((history) => {
  ...
  const enterEdges = statusItems.filter(
    (item) => item.toString && inProgressStatusNames.includes(item.toString)
      && item.fromString && !inProgressStatusNames.includes(item.fromString),
  )
  const leaveEdges = statusItems.filter(
    (item) => item.fromString && inProgressStatusNames.includes(item.fromString)
      && item.toString && !inProgressStatusNames.includes(item.toString),
  )
  if (enterEdges.length !== leaveEdges.length) {
    throw new Error(`Inconsistent in-progress changelog history encountered. Enter edge count: ${enterEdges.length}. Leave edge count: ${leaveEdges.length}`)
  }
...
export const exportIssues = (issues: Issue[], includedStatus: IssueStatus[]) => {
  ...
  issues.forEach((issue) => {
    const exportableIssue = addExportedTimeProperties(issue, inProgressStatusNames);
    const exportedValues = [
      `"${exportableIssue.issueKey}"`,
      `"${exportableIssue.summary}"`,
      `${exportableIssue.startDate ? `"${exportableIssue.startDate?.toISOString()}"` : ""}`,
      `${exportableIssue.endDate ? `"${exportableIssue.endDate?.toISOString()}"` : ""}`,
