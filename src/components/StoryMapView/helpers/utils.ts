...
export const getRndInteger = (min = 0, max = 100000) => Math.floor(Math.random() * (max - min)) + min;

export const getAllActions = (cases: Case[]) => cases.map((_caseColumn) => _caseColumn.actions).flat();

export const getAllSubActionGroups = (cases: Case[]) => getAllActions(cases)
  .map((_action) => _action.subActionGroups)
  .flat();

export const getAllSubActions = (cases: Case[]) => getAllSubActionGroups(cases)
...
): Case[] => produce(cases, (draft) => {
  draft.forEach((_case) => _case.actions.forEach((_action) => {
